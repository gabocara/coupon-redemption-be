...
export const buyService = async (userId: string, amount: number, code: string) => {
  ...
  try {
    ...
    //const newGiftCard = await assignGiftCard(userId, giftCardCode); // Esto se usó para probar la craecion de cupones
    ...
    return { finalAmount, giftCard: giftCardCode }; // Retornar el resultado
  } catch (error: any) {
